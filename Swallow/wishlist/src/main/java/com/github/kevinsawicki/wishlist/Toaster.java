...
public class Toaster {
  ...
  /**
   *Show message (resId) with custom toast(viewId) duration long
   * @param activity
   * @param resId
   * @param viewId
   */
  public static void showCustomLong(final Activity activity,final int resId,final int viewId){
    if(activity == null)
      return;
    showCustom(activity, resId, viewId, Toast.LENGTH_LONG);
  }
   ...
   * @param viewId
   */
  public static void showCustomShort(final Activity activity,final int resId,final int viewId){
    if(activity == null)
      return;
    showCustom(activity, resId, viewId, Toast.LENGTH_SHORT);
  }

  private static void showCustom(Activity activity, int resId, int viewId,int duration) {
    final Context context = activity.getApplication();
    final  Toast t = new Toast(context);
    t.setView(LayoutInflater.from(context).inflate(viewId,null));
    t.setText(resId);
